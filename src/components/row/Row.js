...
function Row({
  category, children, isCinemaximumRow, isColumn,
}) {
        ...
        {movieKeys ? movieKeys.map(key => {
          ...
          return (
            <Link key={movie.id} className={isColumn ? styles['column-link'] : styles.link} to={{ pathname: `movie/${movie.id}`, state: { movie } }}>
              <img
                key={movie.id}
                className={isCinemaximumRow ? styles.imageLarge : styles.image}
                src={`${IMAGE_URL}${isCinemaximumRow ? movie.poster_path : movie.backdrop_path}`}
                alt={movie.name}
              />
            </Link>
          );
